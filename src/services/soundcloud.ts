...
const login = async () => {
  ...
  user = connection.client({ access_token: token })
  return user
}
...
export const createOrUpdatePlaylist = async (data, previous) => {
  await login()
...
export const like = async (trackId: string) => {
  await login()
...
export const likeFromIFTTT = async (url: string) => {
